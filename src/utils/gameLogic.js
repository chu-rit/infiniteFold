...
export function getSymmetricIndex(index, foldDepth, boardSize = BOARD_SIZE) {
  if (foldDepth === 1) {
    if (index === 0) return 1;
...
export function canFold(board, direction, depth) {
  const moves = [];
  const mismatches = [];
  ...
  // Check if fold source area has any blocks
  ...
  return { 
    ...
    moves, 
    mismatches,
    isEmpty: !hasBlocksInFoldArea && mismatches.length === 0 // Empty fold area (no blocks to fold)
  };
}

// Execute a fold
export function executeFold(board, direction, depth) {
  ...
  // Sort: process outer cells first
...
export function getFoldPreview(board, direction, depth) {
